...
src_filename = 'Source_data/DEM/TPI.tif'
...
for year in tqdm(range(start_year, end_year + 1)):
    ...
    FPA_FOD_TPI = geemap.csv_to_gdf(in_csv = FPA_FOD_file_list[0],
                                    latitude = 'LATITUDE',
                                    longitude = 'LONGITUDE').to_crs('EPSG: 5070')
    FPA_FOD_TPI.to_file(filename = 'Source_data/DEM/TPI_pnt.shp')

    shp_filename = 'Source_data/DEM/TPI_pnt.shp'

    ds = ogr.Open(shp_filename)
    lyr = ds.GetLayer()

    FPA_FOD_TPI['TPI'] = None
    i = 0
    ...
    for feat in lyr:
        try:
            ...
            FPA_FOD_TPI.loc[i, 'TPI'] = intval[0]
            i = i + 1
        except:
            i = i + 1
            pass
    ...
    FPA_FOD_TPI.to_csv(path_or_buf = f'TPI/{year}_FPA_FOD_TPI_pt.csv', sep = ',', index = False)
