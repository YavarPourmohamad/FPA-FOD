...
src_filename = 'Source_data/Global_Human_Modification/lulc-human-modification-terrestrial-systems_geographic.tif'
...
for year in tqdm(range(start_year, end_year + 1)):
    ...
    FPA_FOD_GHM = geemap.csv_to_gdf(in_csv = FPA_FOD_file_list[0],
                                    latitude = 'LATITUDE',
                                    longitude = 'LONGITUDE')
    FPA_FOD_GHM.to_file(filename = 'Source_data/Global_Human_Modification/GHM_pnt.shp')

    shp_filename = 'Source_data/Global_Human_Modification/GHM_pnt.shp'

    ds = ogr.Open(shp_filename)
    lyr = ds.GetLayer()

    FPA_FOD_GHM['GHM'] = None
    i = 0
    ...
    for feat in lyr:
        try:
            ...
            FPA_FOD_GHM.loc[i, 'GHM'] = intval[0]
            i = i + 1
        except:
            i = i + 1
            pass
    
    FPA_FOD_GHM = FPA_FOD_GHM.drop(labels = 'geometry', axis = 1)
    FPA_FOD_GHM.to_csv(path_or_buf = f'GHM/{year}_FPA_FOD_GHM_pt.csv', sep = ',', index = False)
